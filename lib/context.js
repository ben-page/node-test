...
const asyncHook = async_hooks.createHook({
    init(asyncId, type, triggerAsyncId, resource) {
        ...
        if (executionAsyncId > 1) {
            const callbacks = asyncCallbacks.get(executionAsyncId);
            if (callbacks)
                callbacks[childTraces].add(asyncId);
        }
...
function removeFromTraces(asyncId) {
    ...
    //remove from parent context, if one exists
    const callbacks = asyncCallbacks.get(data.parent);
    if (!callbacks)
        return;
    callbacks[childTraces].delete(asyncId);

    //the parent has no outstanding async resources
    if (callbacks[childTraces].size === 0) {
