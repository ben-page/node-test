...
const stackTraces = Symbol('stackTraces');
...
Assert.prototype.throws = async function(fn, arg) {
    ...
    this[stackTraces].unshift(stackTrace);
    let result;
    try {
        result = util.runAsync(stackTrace, fn);
        
    } catch (err) {
        if (testErrorFn) {
            const result2 = testErrorFn(err);
            if (util.isPromise(result2))
                this[test].addPromise(result2, [stackTrace].concat(this[stackTraces]));
        }
    } finally {
        this[stackTraces].shift();
    }
    ...
    if (util.isPromise(result)) {
        ...
        this[test].addPromise(handled, [stackTrace].concat(this[stackTraces]));
    }
};
...
Assert.prototype.notThrows = function(fn, message) {
    ...
    if (util.isPromise(result))
        this[test].addPromise(result, [stackTrace].concat(this[stackTraces]));
};
...
Assert.prototype.async = function (...args) {
    ...
    const callback = (...args2) => {
        ...
        if (fulfilled)
            util.throwError('callback executed after test completed', Assert.prototype.async);
    ...
    this[test].addPromise(p, [stackTrace].concat(this[stackTraces]));
